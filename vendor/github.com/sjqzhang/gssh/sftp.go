...
func (c *Client) downloadFile(remoteFile, local string) error {
	...
	if c.Size(remoteFile) > 1000 {
		...
		err := ioutil.WriteFile(localFile+".md5", []byte(rsum), 0755)
		if err != nil {
			return err
		}
		if FileExist(localFile) {
			if rsum != "" {
				...
				if lsum == rsum {
					//log.Info(c.Host, "sftp: 文件与本地一致，跳过下载！", localFile)
					return nil
				}
				//log.Info(c.Host, "sftp: 正在下载 ", localFile)
			}
		}
	}
...
func (c *Client) downloadDir(remote, local string) error {
	...
	for walker.Step() {
		...
		err = c.downloadFile(walker.Path(), localPath)
		if err != nil {
			return err
		}
	}
...
func (c *Client) UploadFile(localFile, remote string) error {
	...
	log.Println(c.Host, "sftp: UploadFile", localFile, remoteFile)
	if info.Size() > 1000 {
		// 1. 检测远程是否存在
		rsum := c.Md5File(remoteFile)
		if rsum != "" {
			lsum, _ := Md5File(localFile)
			if lsum == rsum {
				log.Println(c.Host, "sftp: 文件与本地一致，跳过上传！", localFile)
				return nil
			}
			log.Println(c.Host, "sftp: 正在上传 ", localFile)
		}
	}

	// 目录不存在,则创建 remoteDir
	if _, err := c.SFTPClient.Stat(remoteDir); err != nil {
		log.Println(c.Host, "sftp: Mkdir all", remoteDir)
		err := c.MkdirAll(remoteDir)
		if err != nil {
			return err
		}
	}
...
func (c *Client) UploadDir(localDir string, remoteDir string) (err error) {
	...
	log.Println(c.Host, "sftp: UploadDir", localDir, remoteDir)

	rootDst := strings.TrimSuffix(remoteDir, "/")
	if c.IsFile(rootDst) {
		err := c.SFTPClient.Remove(rootDst)
		if err != nil {
			log.Println(c.Host, "sftp: SFTPClient.Remove", err)
		}
	}
	...
	walkFunc := func(path string, info os.FileInfo, err error) error {
		...
		if finalDst == remoteDir {
			log.Println(c.Host, "sftp: ", remoteDir, "--->", finalDst)
			return nil
		}
		...
		if info.IsDir() {
			...
			if err != nil {
				log.Println(c.Host, "sftp: MkdirAll", err)
			}
...
func (c *Client) RemoveDir(remoteDir string) error {
	...
	if err != nil {
		log.Println(c.Host, fmt.Sprintf("sftp: remove remote dir: %s err: %v\n", remoteDir, err))
		return err
	}
	for _, file := range remoteFiles {
		subRemovePath := path.Join(remoteDir, file.Name())
		if file.IsDir() {
			_ = c.RemoveDir(subRemovePath)
		} else {
			_ = c.RemoveFile(subRemovePath)
		}
	}
	err = c.SFTPClient.RemoveDirectory(remoteDir) //must empty dir to remove
	if err != nil {
		log.Println(c.Host, fmt.Sprintf("sftp: RemoveDirectory: %s err: %v\n", remoteDir, err))
		return err
	}
	log.Println(c.Host, fmt.Sprintf("sftp: remove remote dir: %s ok\n", remoteDir))
	return nil
}

//RemoveAll 递归删除目录，文件
func (c *Client) RemoveAll(remoteDir string) error {
	_ = c.RemoveDir(remoteDir)
	return nil
}
...
func (c *Client) Mkdir(path string, fi os.FileInfo) error {
	log.Println(c.Host, fmt.Sprintf("[DEBUG] sftp: creating dir %s", path))
