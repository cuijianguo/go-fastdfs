...
func (c *Client) Run(cmd string) {
	...
	if err != nil {
		log.Println(c.Host, fmt.Sprintf("exec command:%v error:%v\n", cmd, err))
	}
	log.Println(c.Host, fmt.Sprintf("Waiting for command:%v to finish...\n", cmd))
	//阻塞等待fork出的子进程执行的结果，和cmd.Start()配合使用[不等待回收资源，会导致fork出执行shell命令的子进程变为僵尸进程]
	err = session.Wait()
	if err != nil {
		log.Println(c.Host, fmt.Sprintf(":Command finished with error: %v\n", err))
	}
}
